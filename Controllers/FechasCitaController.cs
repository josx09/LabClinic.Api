...
[ApiController]
[Route("api/fechas-cita")]
[Authorize]
public class FechaCitaController : ControllerBase {
  private readonly LabDbContext _db;
  public FechaCitaController(LabDbContext db){ _db=db; }

  [HttpGet]
  public async Task<IActionResult> Get([FromQuery] PagedQuery q){
    var set = _db.FechasCita.AsQueryable();
    if(!string.IsNullOrWhiteSpace(q.Search)){
      set = set.Where(x => (x.Estado.ToString().Contains(q.Search)));
    }
  ...
  public async Task<IActionResult> GetById(int id){
    var x = await _db.FechasCita.FindAsync(id);
    return x is null ? NotFound() : Ok(x);
  }

  [HttpPost]
  public async Task<IActionResult> Create(FechaCita x){
    _db.FechasCita.Add(x); await _db.SaveChangesAsync();
    return CreatedAtAction(nameof(GetById), new{id = (x as dynamic).Id}, x);
  }

  [HttpPut("{id:int}")]
  public async Task<IActionResult> Update(int id, FechaCita x){
