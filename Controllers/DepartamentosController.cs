...
public class DepartamentosController : ControllerBase {
  ...
  public DepartamentosController(LabDbContext db){ _db=db; }

  [HttpGet]
  public async Task<IActionResult> Get([FromQuery] PagedQuery q){
    var set = _db.Departamentos.AsQueryable();
  ...
  public async Task<IActionResult> GetById(int id){
    var x = await _db.Departamentos.FindAsync(id);
    return x is null ? NotFound() : Ok(x);
  }

  [HttpPost]
  public async Task<IActionResult> Create(Departamento x){
    _db.Departamentos.Add(x); await _db.SaveChangesAsync();
    return CreatedAtAction(nameof(GetById), new{id = (x as dynamic).Id}, x);
  }

  [HttpPut("{id:int}")]
  public async Task<IActionResult> Update(int id, Departamento x){
